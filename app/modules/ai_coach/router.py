...
from app.modules.ai_coach.schema import ChatRequest_aicoach, ResetRequest_aicaoch, ChatState
...
async def stream_response(req: ChatRequest_aicoach, state: ChatState):
    stream_start = time.time()
    final_reply = ""
    final_state = state
    final_source = "coach_engine_v2"
    chunk_count = 0
    ...
    try:
        async for item in process_chat_aicoach_stream(req, state):
            ...
            if item_type == "chunk":
                ...
                payload = json.dumps({"type": "chunk", "text": text}, ensure_ascii=False)
...
async def reset_chat(payload: ResetRequest_aicaoch):
    ...
    return {"status": "ok", "state": serialize_state(state)}